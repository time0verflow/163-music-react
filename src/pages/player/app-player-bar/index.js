...
import { getSizeImage, formatDate, getPlayUrl } from '@/utli/format-utlis'
import {
  // getSongDetailAction,
...
const style = memo(() => {
  ...
  // props/state
  const [currentTime, setCurrentTime] = useState(0); // 用于保存当前播放的时间ms
  const [isShowBar, setIsShowBar] = useState(false); // 是否显示音量播放条
  const [progress, setProgress] = useState(0); // 滑块进度1-100
